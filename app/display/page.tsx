...
export default function Display() {
  ...
  const habilitarAudio = () => {
    // Criar AudioContext com interação do usuário
    const ctx = new (window.AudioContext || (window as any).webkitAudioContext)();
    setAudioContext(ctx);
    setAudioHabilitado(true);

    // Tocar um beep de teste
    tocarBeepTeste(ctx);
  };

  const tocarBeepTeste = (ctx: AudioContext) => {
  ...
  const tocarSom = (senha: string) => {
    if (!audioHabilitado || !audioContext) {
      console.log('Áudio não habilitado. Clique no botão para ativar.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 flex flex-col">
      {/* Botão de Habilitar Áudio */}
      {!audioHabilitado && (
        <div className="fixed top-4 right-4 z-50">
          <button
            onClick={habilitarAudio}
            className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center space-x-2 animate-pulse"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
            </svg>
            <span className="font-bold">Ativar Som</span>
          </button>
        </div>
      )}
      ...
      {audioHabilitado && (
        <div className="fixed top-4 right-4 z-50">
          <div className="bg-green-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center space-x-2">
            ...
            <span className="text-sm font-semibold">Som Ativo</span>
          </div>
        </div>
      )}
